...
#Définit un labyrinthe de nblignes lignes et nbcolonnes colonnes
#Retourne g, le labyrinthe (sous la forme d'un tableau à trois dimensions)
def defineMazeTest(nblignes,nbcolonnes):
    random.seed()
    ...
    for i in range(nblignes):
        for j in range(nbcolonnes):
            z=np.random.uniform(0,1)
            if z < pmur:
                g[i,j] = np.zeros(2, dtype=np.int)
            if z > pmur:
                g[i,j,0] = random.choice(weight)
                g[i,j,1] = 10 * random.choice(weight)
    for k in range(2):
        g[0,0,k] = random.choice(weight)
        g[nblignes-1,nbcolonnes-1,k] = random.choice(weight)
    while not estFinissable(g):
...
def defineMazeAsym(nblignes, nbcolonnes):
    random.seed()
    g= np.zeros((nblignes,nbcolonnes,2), dtype=np.int)
    for i in range(nblignes):
        for j in range(nbcolonnes):
...
def resolutionMultiRegretMinMax(grille, gamma, proba, nbCriteres, nblignes, nbcolonnes):
    (A, b, obj) = dualRegretMinMax(grille, gamma, proba, nbCriteres)
    v, m, t = gurobiMultiMinMax(A, b, obj, nblignes, nbcolonnes)
    pol = politique(v, grille)
    return pol
...
def testTempsCriteres(gamma, proba, nbIter):
    moyTmp = np.zeros(10)
    moyIter = np.zeros(10)
    for i in range(10):
        for j in range(nbIter):
            g = defineMaze(15, 15, (i+1)*2)
            (A, b, obj) = dualMinMax(g, gamma, proba, (i+1)*2)
            v, m, t = gurobiMultiMinMax(A, b, obj, 15, 15)
            moyTmp[i] += t
            moyIter[i] += m.getAttr(GRB.Attr.IterCount)
        moyTmp[i] /= nbIter
        moyIter[i] /= nbIter
    return moyTmp, moyIter

def testTempsTailleMinMax(gamma, proba, nbcrit, nbIter):
    moyTmp = np.zeros(10)
    moyIter = np.zeros(10)
    for i in range(10):
        for j in range(nbIter):
            g = defineMaze((i+1)*3, (i+1)*3, nbcrit)
            (A, b, obj) = dualMinMax(g, gamma, proba, nbcrit)
            v, m, t = gurobiMultiMinMax(A, b, obj, (i+1)*3, (i+1)*3)
            moyTmp[i] += t
            moyIter[i] += m.getAttr(GRB.Attr.IterCount)
        moyTmp[i] /= nbIter
        moyIter[i] /= nbIter
...
'''
#Test plotPols
g=defineMaze(nblignes, nbcolonnes, 2)
print g
pol1 = resolutionMultiSomme(g, gamma, probaTransition, 2, nblignes, nbcolonnes)
print pol1
pol2 = resolutionMultiRegretMinMax(g, gamma, probaTransition, 2, nblignes, nbcolonnes)
...
plotPols(ex1, ex2, REGRET)'''

#Test temps
moyT, moyI = testTempsCriteres(gamma, probaTransition, 5)
print moyT
print moyI
...
#gamma= 0.8
#probaTransition=0.8
#pol =   resolutionMultiRegretMinMax(g, gamma, probaTransition, nbcri,nbl,nbc)
